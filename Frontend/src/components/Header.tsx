  ...
  const NavLinks = () =>
    isLoggedIn ? (
      <>
        ...
        <Link href={config.auth.profilePath} className="hover:text-primary transition-colors">
          Profile
        </Link>
      ...
      <>
        <Link href={config.auth.loginPath} className="hover:text-primary transition-colors">
          Login
        </Link>
        <Link href={config.auth.registerPath} className="hover:text-primary transition-colors">
          Register
        </Link>
      </>
    );
  ...
  return (
    <nav className="fixed top-0 left-0 w-full border-b border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-900/70 backdrop-blur-md z-50">
      <div className="max-w-6xl mx-auto flex justify-between items-center p-4">
        {/* Logo */}
        <Link href="/" className="font-bold text-lg tracking-wide flex items-center gap-2 text-gray-900 dark:text-gray-100">
          🎵 <span className="hidden sm:inline">MyMusic</span>
        </Link>

        {/* Desktop menu (safe, rendered also on server) */}
        <div className="hidden md:flex gap-6 items-center text-gray-800 dark:text-gray-200">
          <NavLinks />
          {mounted && <ModeToggle />} {/* Theme toggle only when mounted */}
        </div>

        {/* Mobile menu button (rendered only after mount) */}
        {mounted && (
          <button
      ...
      {/* Mobile menu - also only after mount */}
      {mounted && menuOpen && (
